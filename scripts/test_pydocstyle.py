#!/usr/bin/env python3
"""
Parse pydocstyle text output into FixSignals and write a readable TXT dump.

Run from your repo root (so imports resolve), e.g.:
  python scripts/test_pydocstyle.py
"""
...
# TODO: Update with your actual pydocstyle output file path
pydocstyle_output_path = Path("")

# Output file paths
output_dir = Path(__file__).parent / "test-outputs"
context_output = output_dir / "pydocstyle-fix-signals-debug.txt"
fix_planner_output = output_dir / "pydocstyle-fix-plan.json"

# TODO: Update with your repo root path
repo_root = ""
...
def main() -> int:
    ...
    if PARSE_PYDOCSTYLE_AND_OUTPUT:
        # Parse pydocstyle text output
        pydocstyle_output = Path(pydocstyle_output_path).read_text(encoding="utf-8")
        signals = parse_pydocstyle_results(pydocstyle_output, repo_root=repo_root)

        # Output fix signals to a file for inspection
        out_lines = [f"Parsed {len(signals)} pydocstyle FixSignals:\n"]
        ...
        planner = FixPlanner(llm_provider="anthropic", repo_root=repo_root)
        ...
        with (fix_planner_output).open("w", encoding="utf-8") as f:
            ...
            for i, plan in enumerate(fix_plans_from_llm):
                print(f"\n--- Writing pydocstyle Fix Plan {i} to JSON ---")
