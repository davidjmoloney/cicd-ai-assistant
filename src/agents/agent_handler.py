...
class AgentHandler:
    ...
    def _build_user_prompt(self, context: dict[str, Any]) -> str:
        ...
        prompt_parts = [
            f"## Signal Group",
            f"Tool: {group_info.get('tool_id', 'unknown')}",
            f"Signal Type: {group_info.get('signal_type', 'unknown')}",
            f"Number of signals: {group_info.get('group_size', len(signals))}",
            "",
            "## Signals to Fix",
            "",
        ]
        ...
        for i, signal_data in enumerate(signals, 1):
            ...
            code_ctx = signal_data.get("code_context", {})
            fix_ctx = signal_data.get("fix_context", {})
            file_error = signal_data.get("file_read_error")

            prompt_parts.append(f"### Signal {i}")
            prompt_parts.append(f"- File: {sig.get('file_path', 'unknown')}")
            prompt_parts.append(f"- Rule: {sig.get('rule_code', 'N/A')}")
            prompt_parts.append(f"- Severity: {sig.get('severity', 'unknown')}")
            prompt_parts.append(f"- Message: {sig.get('message', '')}")

            if sig.get("docs_url"):
                prompt_parts.append(f"- Docs: {sig.get('docs_url')}")
            ...
            if span:
                ...
                prompt_parts.append(
                    f"- Location: row {start.get('row')}, col {start.get('column')} "
                    f"to row {end.get('row')}, col {end.get('column')}"
                )

            if file_error:
                prompt_parts.append(f"- File read error: {file_error}")
            else:
                # Add code context
                window = code_ctx.get("window")
                if window:
                    prompt_parts.append("")
                    prompt_parts.append(
                        f"#### Code Window (rows {window.get('start_row')}-{window.get('end_row')}):"
                    )
                    prompt_parts.append("```python")
                    # Add line numbers to code
                    lines = window.get("text", "").splitlines()
                    start_row = window.get("start_row", 1)
                    for j, line in enumerate(lines):
                        prompt_parts.append(f"{start_row + j:4d} | {line}")
                    prompt_parts.append("```")

                imports = code_ctx.get("imports")
                if imports:
                    prompt_parts.append("")
                    prompt_parts.append("#### Import Block:")
                    prompt_parts.append("```python")
                    prompt_parts.append(imports.get("text", "").rstrip())
                    prompt_parts.append("```")

                enclosing = code_ctx.get("enclosing_function")
                if enclosing:
                    prompt_parts.append("")
                    prompt_parts.append(
                        f"#### Enclosing Function (rows {enclosing.get('start_row')}-{enclosing.get('end_row')}):"
                    )
                    prompt_parts.append("```python")
                    prompt_parts.append(enclosing.get("text", "").rstrip())
                    prompt_parts.append("```")

            # Add tool-provided fix if available
            if fix_ctx.get("exists"):
                prompt_parts.append("")
                prompt_parts.append("#### Tool-Provided Fix:")
                prompt_parts.append(f"- Applicability: {fix_ctx.get('applicability', 'unknown')}")
                if fix_ctx.get("tool_message"):
                    prompt_parts.append(f"- Message: {fix_ctx.get('tool_message')}")

                edits = fix_ctx.get("edits", [])
                if edits:
                    prompt_parts.append("- Edits:")
                    for edit in edits:
                        edit_span = edit.get("span", {})
                        start = edit_span.get("start", {})
                        end = edit_span.get("end", {})
                        content = edit.get("content", "")
                        content_preview = repr(content[:50] + "..." if len(content) > 50 else content)
                        prompt_parts.append(
                            f"  - Replace ({start.get('row')}:{start.get('column')}) to "
                            f"({end.get('row')}:{end.get('column')}) with {content_preview}"
                        )
        ...
        prompt_parts.append("---")
        prompt_parts.append("")
        prompt_parts.append("Generate a fix plan JSON for all signals above.")
